            ...
            <PopoverContent className='ml-7 p-4 '>
                ...
                <div>
                    {menu.map((item,index)=>(
                        ...
                        <h2 key={index} className='flex gap-2 items-center p-2 hover:bg-gray-100 rounded-lg text-sm cursor-pointer' onClick={()=>onMenuClick(item)}> <item.icon className='h-4 w-4'/> {item.name}</h2>
                    ))}
                    <LogoutLink>
                        <h2 className='flex gap-2 items-center p-2 hover:bg-gray-100 rounded-lg text-sm cursor-pointer'> <LogOut className='h-4 w-4'/> Logout</h2>
                    </LogoutLink>
                </div>
                <Separator className='mt-2 bg-slate-100'/>
